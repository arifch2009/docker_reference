...
func writeStatus(requestedTag string, out progress.Output, layersDownloaded bool) {
	if layersDownloaded {
		progress.Message(out, "", "Status:(encoded by arif) Downloaded newer image for "+requestedTag)
	} else {
		progress.Message(out, "", "Status:(encoded by arif) Image is up to date for "+requestedTag)
	}
}
